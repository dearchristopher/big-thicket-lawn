  ...
  return (
    <>
      {value.map((block, index) => {
        ...
        const typedBlock = block as {style?: string; children?: Array<{text?: string; marks?: string[]}>}
        ...
        if (style === 'h1') return <h1 key={index} className="text-3xl font-bold mb-4">{children}</h1>
        if (style === 'h2') return <h2 key={index} className="text-2xl font-bold mb-3">{children}</h2>
        if (style === 'h3') return <h3 key={index} className="text-xl font-bold mb-2">{children}</h3>
        if (style === 'blockquote') return <blockquote key={index} className="border-l-4 border-green-500 pl-4 italic my-4">{children}</blockquote>
              ...
              <div className="bg-green-100 rounded-lg p-8 flex items-center justify-center">
                <div className="text-center">
                  <div className="text-6xl mb-4">
                    <img src="/images/terrytodd.jpg" alt="Big Thicket Lawn Services - Owners" className="rounded" />
                  </div>
